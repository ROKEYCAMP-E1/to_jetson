...
class ImageProcessor(Node):
    def __init__(self):
        ...
        # 타이머 설정 (10초 주기)
        self.timer = self.create_timer(300.0, self.check_service_terminate)
    ...
    def listener_callback(self, msg):
        ...
        # 이미지 해상도 조정 (1280x720),(640, 360),(320, 180)
        frame = cv2.resize(frame, (320, 180))

        # YOLOv8 모델로 객체 탐지 및 트래킹 수행
        results = self.model.track(source=frame, conf=0.9, persist=True)
        ...
        x_s = int(image_width*(150/1280))
        x_e = int(image_width*(30/1280))
        y = int(image_height*(200/720))
        # 그린존 영역 추가
        cv2.line(frame_with_boxes, (center_x - x_s, 0), (center_x - x_e, image_height), (0, 255, 0), 2)
        cv2.line(frame_with_boxes, (center_x + x_s, 0), (center_x + x_e, image_height), (0, 255, 0), 2)
        cv2.line(frame_with_boxes, (0, center_y + y), (image_width, center_y + y), (0, 255, 0), 2)
        ...
        for i, box in enumerate(boxes):
            ...
            # 첫 번째 대각선에서 box_center_y에 해당하는 x값 계산
            x_first = (x_s - x_e) * (box_center_y / image_height) + (center_x - x_s)
            # 두 번째 대각선에서 box_center_y에 해당하는 x값 계산
            x_second = (x_e - x_s) * (box_center_y / image_height) + (center_x + x_s)
