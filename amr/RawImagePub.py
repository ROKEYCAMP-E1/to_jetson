...
class RawImagePublisher(Node):
    def __init__(self):
        ...
        # 타이머 생성: 30FPS로 퍼블리싱
        self.timer = self.create_timer(1 / 15.0, self.publish_image)
        self.get_logger().info('Timer set to 30 FPS.')#낮춰도될지도

        # 카메라 초기화 (/dev/video0)
        self.cap = cv2.VideoCapture("/dev/video0")
        # self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)  # 해상도 설정 (가로)
        # self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)  # 해상도 설정 (세로)
        # self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)  # 가로 해상도 설정
        # self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 360)  # 세로 해상도 설정
        self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, 320)  # 가로 해상도 설정
        self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 180)  # 세로 해상도 설정
